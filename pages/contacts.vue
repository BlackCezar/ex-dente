...
useSeo(data.value.contactPage.data.attributes.title, data.value.contactPage.data.attributes.seo)
</script>

<template>
    <div class="vertical-padding bg-white">
        <div class="container mx-auto" v-if="data?.contactPage?.data?.attributes">
            <BreadCrumbs is-dark :list="breadCrumbs" class="lg:mb-[3.75rem] mb-7" />
            ...
            <div class="mb-16 lg:mb-[7.5rem]">
                <div class="relative">
                    <yandex-map v-model="map" class="ymaps" :settings="{
                        location: {
                        ...
                        <yandex-map-entity>
                            <div class="map-list">
                                <div class="map-list-inner flex flex-col">
                                    <section class="px-7 font-serif h4 pt-[2.75rem] pb-8 lg:block hidden">
                                        Филиалы exellentDENT
                                    </section>
                                    <div class="flex flex-col overflow-y-auto max-h-auto">
                                        <article v-for="item of data.contactPage.data
                                            .attributes.clinics.data" :key="item.id"
                                            ...
                                            <address class="not-italic text-accent text-opacity-70 lg:text-[1.375rem]">
                                                {{ item.attributes.address }}
                                            </address>
                                            <div v-if="item.attributes.phone"
                                                class="phone-blocks text-accent text-opacity-50 lg:text-[1.125rem]">
                        ...
                        <template v-for="clinic of data.contactPage.data.attributes
                            .clinics.data" :key="clinic.id">
                            <yandex-map-marker :settings="{
                                ...
                                <div @click="selectClinic(clinic)" class="marker" :class="{
                    ...
                    <Transition name="fade">
                        <div v-if="isLoading"
                            class="absolute bg-white left-0 top-0 w-full h-full flex items-center justify-center">
                            <UiPageSpinner />
                        </div>
                    </Transition>
                </div>
            ...
            <div class="mb-[4.5rem] lg:mb-[7.5rem]">
                <div class="flex flex-col lg:gap-y-16 gap-y-[3.75rem]">
                    <div v-for="item of data.contactPage.data.attributes.clinics
                        .data" :key="item.id" class="grid text-accent lg:gap-10 gap-4 grid-cols-1 lg:grid-cols-5">
                        <section
                            ...
                            <span v-if="item.attributes.busStation" class="text-base font-sans">Автобусная остановка: {{
                                item.attributes.busStation }}</span>
                        </section>
                        <section v-if="item.attributes.phone" class="lg:col-start-2 lg:col-end-3 flex flex-col gap-1">
                            <span class="lg:hidden text-accent text-[0.875rem] text-opacity-50">Телефон</span>
                            <div class="phone-blocks lg:text-[1.375rem]" v-if="item.attributes.phone">
                                <RenderBlocks :text="item.attributes.phone" />
                            </div>
                        </section>
                        <section v-if="item.attributes.email" class="lg:col-start-4 lg:col-end-5 flex flex-col gap-1">
                            <span class="lg:hidden text-accent text-[0.875rem] text-opacity-50">Почта</span>
                            <a class="lg:text-[1.375rem]" :href="'mailto:' + item.attributes.email">{{ item.attributes.email
                            }}</a>
                        </section>
                        <section class="lg:col-start-3 lg:col-end-4 mb-4 lg:mb-0 lg:row-start-1 flex flex-col gap-1">
                            <span class="lg:hidden text-accent text-[0.875rem] text-opacity-50">Режим работы</span>
                            <div class="content" v-if="item.attributes.workingHours">
                                <RenderBlocks :text="item.attributes.workingHours"></RenderBlocks>
                            </div>
                        </section>
                                ...
                                <ul class="flex gap-3">
                                    <li v-for="link of item.attributes.socials">
                                        <a :href="link.url" :title="link.label"
                                            ...
                                            <img class="grayscale contrast-200 invert" :src="config.assetsUri +
